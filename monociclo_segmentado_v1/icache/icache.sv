...
module icache(
	...
	//Señales de memoria
	output										hit_o,
	output		[ADDR_SIZE-1:0]			pc_o
);

	wire											hit_w;
	wire			[ICACHE_LINE_SIZE-1:0]	inst_w;
	...
	initial
	begin
		$readmemh("burbuja2025_inst.hex",	memoria); // Cambiar por mi archivo de quick
	...
	*/
	...
	assign	tag_mw 	= tag[idx_w];
	assign	valid_w	= valid[idx_w];
	
	assign 	hit_o = ((tag_mw == tag_iw) && valid_w) ? 1'b1 : 1'b0;
	assign	inst_w 	= (hit_o) ? memoria[idx_w] : {ICACHE_LINE_SIZE{1'b0}};
	assign	inst_o	= (pc_i[ILINE_BYTE_OFFSET-1]) ? inst_w[63:32] : inst_w[31:0];
	assign 	pc_o		=	pc_i;
